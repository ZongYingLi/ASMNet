...
def ConvBlock(kernel_size, in_channels, out_channels, stride=1, pad_type='reflect', dropout=None,
    ...
    layers += get_dropout_layer(dropout)       # None
...
class ResBlock(nn.Module):
    ...
    def __init__(self, kernel_size, channels, stride=1, pad_type='zero', norm='none', acti='relu'):
        ...
        layers = []
        layers += ConvBlock(kernel_size, channels, channels, stride=stride, pad_type=pad_type, norm=norm, acti=acti)
        layers += ConvBlock(kernel_size, channels, channels, stride=stride, pad_type=pad_type, norm=norm, acti='none')
