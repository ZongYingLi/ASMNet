...
clip_kinematic_chain = [[0, 2, 5, 8, 11], [0, 1, 4, 7, 10], [0, 3, 6, 9, 12, 15], [9, 14, 17, 19, 21], [9, 13, 16, 18, 20]]
...
# xia， 我加的
xia_kinematic_chain = [[0, 11, 12, 13, 14, 15], [0, 16, 17, 18, 19, 20], [0, 1, 2, 3, 4], [3, 5, 6, 7], [3, 8, 9, 10]]
...
def plot_3d_motion(motion, length, save_path, params, title="", interval=50):
    ...
    # colors = ["#DD5A37", "#D69E00", "#B75A39", "#FF6D00", "#DDB50E"]  # 橘色
    colors = ['#634FA2', '#BB4A9C', '#B0297B', '#BB4A9C', '#634FA2']
    ...
    elif motion.shape[0] == 21:
        kinematic_tree = xia_kinematic_chain
    elif motion.shape[0] == 22:
        kinematic_tree = clip_kinematic_chain
    else:
        kinematic_tree = None
...
def plot_3d_motion_dico(x):
    if len(x) == 5:
        motion, length, content_path, params, kargs = x
        motion = motion / 20  # for xia
        plot_3d_motion(motion, length, content_path, params, **kargs)
    else:
        motion, length, action2style, content_path, style_path, params, kargs = x
        motion = motion / 20    # for xia [21, 3, 80]
        action2style = action2style/20
        plot_3d_motion(motion, length, content_path, params, **kargs)
        plot_3d_motion(action2style, length, style_path, params, **kargs)
